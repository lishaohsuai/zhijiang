  ...
  methods: {
    ...
    drawGraph: function() {
      ...
      function DrawSpecialNodes(graph, parent) {
        ...
        d3.selectAll(`.nodes .node`)
          .attr('temp', function(uid) {
            if (!nodedata[uid] || IsSpecialNode(nodedata[uid]) !== 1) {
              return ''
            }
            var node = graph.node(uid)
            if (!node) {
            ...
            d3.select(this)
              .select('rect').style('fill', '#e8c5df')
          })

        // 全连接层
        d3.selectAll(`.nodes .node`)
          .attr('temp', function(uid) {
            if (!nodedata[uid] || IsSpecialNode(nodedata[uid]) !== 2) {
              return ''
            }
            var node = graph.node(uid)
            if (!node) {
        ...
        d3.selectAll(`.nodes .node`)
          .attr('temp', function(uid) {
            if (!nodedata[uid] || IsSpecialNode(nodedata[uid]) !== 3) {
              return ''
            }
            var node = graph.node(uid)
            if (!node) {
        ...
        d3.selectAll(`.nodes .node`)
          .attr('temp', function(uid) {
            if (!nodedata[uid] || IsSpecialNode(nodedata[uid]) !== 4) {
              return ''
            }
            var node = graph.node(uid)
            if (!node) {
              ...
              for (let i = 0; i < tmp.length; i++) {
                ...
                if (initInfo.outNode.indexOf(tmp[i].nodeId) >= 0) {
                  ...
                  if (oldfill) {
                    ...
                    const fill = d3.select('#del__' + tmp[i].nodeId.replace(/\//g, '\\/').replace(/\(/g, '\\(').replace(/\)/g, '\\)')).select('rect').attr('red')
                    d3.select('#del__' + tmp[i].nodeId.replace(/\//g, '\\/').replace(/\(/g, '\\(').replace(/\)/g, '\\)')).select('rect').style('fill', 'blue')
                    d3.select('#del__' + tmp[i].nodeId.replace(/\//g, '\\/').replace(/\(/g, '\\(').replace(/\)/g, '\\)')).select('rect').attr('oldfill', fill)
                  }
                }
